...
unsigned char* createElongatedArray(FHeader image,unsigned int newWidth,float *arrayPSF){
    unsigned char *arrayTarget = calloc(image.height*newWidth*(image.npp/8),sizeof(unsigned char));
    int base = 0;
        float maxCoef = arrayPSF[base];
        float coefTemp = maxCoef;
        float coef1 = 0;
        float remainder = 0;
    for(int i=0;i<image.height;i++){
        maxCoef = arrayPSF[base];
        for(int j=0,a=0;j<(newWidth*(image.npp/8));j++){
            base = i/200;
            if(base<12){
                if( (newWidth- (int)(image.width*arrayPSF[base]))%2 == 0){
                    if(j<((int)((newWidth - (int)(image.width*arrayPSF[base]))/2*(image.npp/8))) || j>((int)(((int)((newWidth - (int)(image.width*arrayPSF[base]))/2)+(int)(image.width*arrayPSF[base]))*(image.npp/8)))){
                        arrayTarget[i*newWidth*image.npp/8+j] = 255;
                        continue;
                    }
                }
                else{
                ...
                arrayTarget[i*newWidth*image.npp/8+j] =  coef1*image.imageArray[i*newWidth*image.npp/8+a] + remainder*(image.imageArray[i*image.width*image.npp/8+a+4]);
                a = a+1;
                j = j+1;
                arrayTarget[i*newWidth*image.npp/8+j] =  coef1*image.imageArray[i*newWidth*image.npp/8+a] + remainder*(image.imageArray[i*image.width*image.npp/8+a+5]);
                j = j+1;                
                a = a+1;
                arrayTarget[i*newWidth*image.npp/8+j] =  coef1*image.imageArray[i*newWidth*image.npp/8+a] + remainder*(image.imageArray[i*image.width*image.npp/8+a]);
                j = j+1;
                a = a+1;
                arrayTarget[i*newWidth*image.npp/8+j] =  coef1*image.imageArray[i*newWidth*image.npp/8+a];
